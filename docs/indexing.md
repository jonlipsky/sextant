...
- **Consumer (single writer).** A single consumer drains the channel **in enqueue order** and is the only stage that resolves occurrence targets through the `SymbolCatalog` (including the `AmbiguousEdgeBindings` count) and writes to SQLite via the Phase 3 `IndexWriteSession`. Distinct from that consumer-only catalog resolution, the assembly→project-id memo (`ResolveTargetProject`) is a `ConcurrentDictionary` precisely because it *is* invoked from the parallel workers (to stamp each occurrence's `TargetProjectId`); its side-effect-free factory (`Solution.GetProject` + the read-only `projectRoslynToId` map) makes concurrent reads safe. That memo is **reset at each project boundary**, so it only ever retains the current project's referenced assembly symbols — all already rooted by that project's live compilation — which keeps the one-compilation-at-a-time memory bound intact instead of accumulating every processed project's assemblies.
